...
const run = (nRows, limit, refuseHard) => {
  const srv = makeServer(limit, 60000);
  const F = new Function('srv', 'nRows', 'refuseHard',
    ...
    '  calls++;\n' +
    '  if (refuseHard && refuseHard.indexOf(o.body.name) >= 0) { const e = new Error("Task name is required."); e.status = 400; throw e; }\n' +
    ...
    '  .then(r => ({ r, calls, waits, ms: srv.at() }));');
  return F(srv, nRows, refuseHard || null);
};
...
run(275, 240).then(o => {
  ...
  ok('and it took more than a minute of simulated time to do it honestly',
     o.ms > 30000,
  ...
  // A REAL refusal must not be retried forever, and must be reported with its reason.
