...
cmd_rm() {
  ...
  if [ "$unmerged" != "0" ]; then
    ...
    local landed=no merged_tree main_tree merge_conflicts="" MERGED_PR=""
    if [ -z "$tip" ]; then
      landed=diverged
      ...
      if [ "$landed" != yes ] && command -v gh >/dev/null 2>&1; then
        local merged_pr
        merged_pr=$(gh pr list --state merged --head "$branch" --json number \
                      --jq '.[0].number' --limit 1 2>/dev/null || true)
